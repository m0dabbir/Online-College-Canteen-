...
app.post("/", (req, res) => {
  ...
  con.query(
    "insert into tenders(name,contact,email,campus,partners,date,reference) values(?,?,?,?,?,?,?);",
    [name, number, email, campus, partners, date, reference],
    (error, result) => {}
  );
});
...
app.post("/loginfet", (req, res) => {
  ...
  con.query(
    `select studentid,password from canteenfet; `,
    (err, result, field) => {
...
app.post("/signupfet", (req, res) => {
  ...
  con.query(
    "insert into canteenfet(name,faculty,department,studentid,mobilenumber,email,password) values(?,?,?,?,?,?,?);",
    [name, faculty, department, studid, number, email, createpass],
    (error, result) => {}
  );
});
...
app.post("/logincentral", (req, res) => {
  ...
  con.query(
    `select studentid,password from canteencentral; `,
    (err, result, field) => {
      let length = result.length;
      while (i < length) {
        if (studid == result[i].studentid) {
          if (password == result[i].password) {
            res.sendFile(
              path.join(__dirname, "pages", "central", "menucentral.html")
            );
          } else {
            res.sendFile(path.join(__dirname, "fail.html"));
          }
...
app.post("/signupcentral", (req, res) => {
  ...
  con.query(
    "insert into canteencentral(name,faculty,department,studentid,mobilenumber,email,password) values(?,?,?,?,?,?,?);",
    [name, faculty, department, studid, number, email, createpass],
    (error, result) => {}
  );
});
...
app.post("/logincastro", (req, res) => {
  ...
  con.query(
    `select studentid,password from canteencastro; `,
    (err, result, field) => {
      let length = result.length;
      while (i < length) {
        if (studid == result[i].studentid) {
          if (password == result[i].password) {
            res.sendFile(
              path.join(__dirname, "pages", "castro", "menucastro.html")
            );
          } else {
            res.sendFile(path.join(__dirname, "fail.html"));
          }
...
app.post("/signupcastro", (req, res) => {
  ...
  con.query(
    "insert into canteencastro(name,faculty,department,studentid,mobilenumber,email,password) values(?,?,?,?,?,?,?);",
    [name, faculty, department, studid, number, email, createpass],
    (error, result) => {}
  );
});
